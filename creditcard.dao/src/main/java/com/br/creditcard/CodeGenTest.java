...
public class CodeGenTest {
    ...
    public static void main(String[] args) {
        ...
        String url = "jdbc:mysql://192.168.180.10:3306/card?zeroDateTimeBehavior=convertToNull&useUnicode=true&characterEncoding=utf8";
        String user = "card";
        String password = "card_2018";
        try {
            Class.forName(driver);
            Connection conn = DriverManager.getConnection(url, user, password);
            if (!conn.isClosed()) {
                Statement statement = conn.createStatement();
                String sql = "select COLUMN_NAME,COLUMN_COMMENT ,DATA_TYPE from information_schema.COLUMNS where table_name = 'c_user'";
